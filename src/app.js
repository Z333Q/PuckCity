...
const CITY_TOKENS = [
  ...
  { name: "New York Islanders", image: "new-island.png" },
  { name: "New York Rangers", image: "new-york.png" },
...
async function refreshData() {
  ...
  CITY_TOKENS.forEach(async (teamToken) => {
...
async function stake(team, amount) {
  const contract = await loadContract();
  const account = await loadAccount();
  const teamToken = CITY_TOKENS.find((t) => t.name === team);
