...
function getUserStakedBalance(address user, string memory teamName) public view returns (uint256) {
        return stakedBalances[user][teamName];
    }

    function getTeamInfo(string memory teamName) public view returns (uint256 totalStaked, uint256 winBalance, uint256 lossBalance) {
        Team storage team = teams[teamName];
        return (team.totalStaked, team.winBalance, team.lossBalance);
    }

    function getRewardInfo(address user, string memory teamName) public view returns (uint256 reward, uint256 penalty) {
        Team storage team = teams[teamName];
        reward = stakedBalances[user][teamName].mul(team.winBalance).div(team.totalStaked);
        penalty = stakedBalances[user][teamName].mul(team.lossBalance).div(team.totalStaked);
